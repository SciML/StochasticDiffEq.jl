@verbosity_specifier SDEVerbosity begin
    toggles = (
        # ODE fields from OrdinaryDiffEqCore
...
## Sensitivity Group (from ODE)
- `sensitivity_vjp_choice`: Messages about VJP choice in sensitivity analysis
...
- `SciMLLogging.All()`: Maximum verbosity
    ...
    SDEVerbosity(; error_control=nothing, performance=nothing, numerical=nothing,
                   sensitivity=nothing, sde_specific=nothing, kwargs...)
...
verbose = SDEVerbosity(
    ...
    max_iters = SciMLLogging.WarnLevel()
)
