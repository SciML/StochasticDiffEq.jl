    ...
    @fastmath if !isempty(tstops)
        ...
        elseif integrator.dtchangeable && !integrator.force_stepfail
            ...
            integrator.dt = @fastmath integrator.tdir * min(abs(integrator.dtcache),
                                          abs(first(tstops) -
                                              integrator.tdir * integrator.t)) # step! to the end
        end
    end
end
...
@inline function savevalues!(integrator::SDEIntegrator,
    ...
    while !isempty(integrator.opts.saveat) && first(integrator.opts.saveat) <= tdir_t # Perform saveat
        ...
        if curt != integrator.t # If <t, interpolate
            ...
            if typeof(integrator.alg) <: Union{StochasticDiffEqCompositeAlgorithm,
...
function DiffEqBase.nlsolve_f(f, alg::StochasticDiffEqAlgorithm)
    f isa SplitSDEFunction && issplit(alg) ? f.f1 : f
end
function DiffEqBase.nlsolve_f(integrator::SDEIntegrator)
...
function oop_generate_W(alg, u, uprev, p, t, dt, f, uEltypeNoUnits)
    ...
    if islin || DiffEqBase.has_jac(f)
        ...
        if u isa StaticArray
            ...
            W = u isa Number ? u :
                LU{LinearAlgebra.lutype(uEltypeNoUnits)}(Matrix{uEltypeNoUnits}(undef, 0,
                                                                                0),
                                                         Vector{LinearAlgebra.BlasInt}(undef,
                                                                                       0),
