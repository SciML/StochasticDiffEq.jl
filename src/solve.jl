...
function DiffEqBase.__init(_prob::Union{DiffEqBase.AbstractRODEProblem, JumpProblem},
                           alg::Union{AbstractRODEAlgorithm, AbstractSDEAlgorithm},
                           timeseries_init = typeof(_prob.u0)[],
                           ...
                           dtmax = eltype(concrete_prob(_prob).tspan)((concrete_prob(_prob).tspan[end] -
                                                                       concrete_prob(_prob).tspan[1])),
                           dtmin = typeof(one(eltype(concrete_prob(_prob).tspan))) <:
                                   AbstractFloat ? eps(eltype(concrete_prob(_prob).tspan)) :
                                   typeof(one(eltype(concrete_prob(_prob).tspan))) <:
                                   Integer ? 0 :
                           ...
                           alias_jumps = Threads.threadid() == 1,
                           kwargs...) where {recompile_flag}
    ...
    elseif typeof(prob) <: DiffEqBase.AbstractRODEProblem && prob.f.mass_matrix != I &&
    ...
    if typeof(prob) <: DiffEqBase.AbstractRODEProblem &&
       typeof(prob.noise) <: NoiseProcess && prob.noise.bridge === nothing && adaptive
    ...
    progress &&
        @logmsg(LogLevel(-1), progress_name, _id=_id = :StochasticDiffEq, progress=0)
    ...
    d_discontinuities_internal = OrdinaryDiffEq.initialize_d_discontinuities(tType,
                                                                             d_discontinuities,
                                                                             tspan)
    ...
    if !(uType <: AbstractArray)
        ...
        elseif prob isa DiffEqBase.AbstractSDEProblem
            if issparse(u)
                rand_prototype = adapt(DiffEqBase.parameterless_type(u),
                                       zeros(randElType, size(noise_rate_prototype, 2)))
            else
    ...
    cache = alg_cache(alg, prob, u, dW, dZ, p, rate_prototype, noise_rate_prototype,
                      jump_prototype, uEltypeNoUnits, uBottomEltypeNoUnits, tTypeNoUnits,
                      uprev, f, t, dt, Val{isinplace(_prob)})
    ...
    integrator = SDEIntegrator{typeof(alg), isinplace(prob), uType,
                               ...
                               typeof(eigen_est), QT,
                               uEltypeNoUnits, typeof(W), typeof(P), rateType, typeof(sol),
                               typeof(cache),
                               FType, GType, CType, typeof(opts), typeof(noise),
                               typeof(last_event_error), typeof(callback_cache),
                               typeof(rate_constants)}(f, g, c, noise, uprev, tprev, t, u,
                                                       p, tType(dt), tType(dt), tType(dt),
                                                       dtcache, tspan[2], tdir,
    ...
    if initialize_integrator
        ...
        save_start &&
            typeof(alg) <: Union{StochasticDiffEqCompositeAlgorithm,
                  StochasticDiffEqRODECompositeAlgorithm} &&
