...
function isadaptive(
        prob, alg::Union{StochasticDiffEqAlgorithm, StochasticDiffEqRODEAlgorithm})
    isadaptive(alg)
end
...
function beta1_default(
        alg::Union{StochasticDiffEqAlgorithm, StochasticDiffEqRODEAlgorithm}, beta2)
...
function OrdinaryDiffEqCore.get_current_alg_autodiff(
        alg::StochasticDiffEqCompositeAlgorithm, cache)
...
@static if isdefined(OrdinaryDiffEqCore, :standardtag)
    function OrdinaryDiffEqCore.standardtag(alg::Union{
...
@static if isdefined(OrdinaryDiffEqCore, :alg_difftype)
    function OrdinaryDiffEqCore.alg_difftype(alg::Union{
            StochasticDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ, Controller},
...
@static if isdefined(OrdinaryDiffEqCore, :concrete_jac)
    function OrdinaryDiffEqCore.concrete_jac(alg::Union{
            StochasticDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ, Controller},
            StochasticDiffEqNewtonAlgorithm{CS, AD, FDT, ST, CJ, Controller}}) where {
            CS, AD, FDT, ST, CJ, Controller}
        CJ
