...
@inline function current_extrapolant(
        t::Number, integrator::DEIntegrator, idxs = nothing, deriv = Val{0})
...
@inline function current_extrapolant(
        t::AbstractArray, integrator::DEIntegrator, idxs = nothing, deriv = Val{0})
    Θ = (t .- integrator.tprev) ./ (integrator.t - integrator.tprev)
    [sde_extrapolant(ϕ, integrator, idxs, deriv) for ϕ in Θ]
end

@inline function current_extrapolant!(
        val, t, integrator::DEIntegrator, idxs = nothingderiv = Val{0})
    Θ = (t .- integrator.tprev) ./ (integrator.t - integrator.tprev)
    ...
    vals = map(idx) do j
        @inbounds begin
            ...
            if continuity === :left
                ...
                i₊ = min(lastindex(ts),
                    OrdinaryDiffEqCore._searchsortedfirst(ts, t, i₊, tdir > 0))
                i₋ = i₊ > 1 ? i₊ - 1 : i₊
            else
                # we have i₋ = i₊ - 1 = 1 if t < ts[1], i₊ = i₋ = lastindex(ts) if t = ts[end],
                # and otherwise i₋ and i₊ satisfy ts[i₋] ≤ t < ts[i₊]
                i₋ = max(1, OrdinaryDiffEqCore._searchsortedlast(ts, t, i₋, tdir > 0))
...
function sde_interpolation(tval::Number, id, idxs, deriv, p, continuity::Symbol = :left)
    ...
    if continuity === :left
        ...
        i₊ = min(
            lastindex(ts), OrdinaryDiffEqCore._searchsortedfirst(ts, tval, 2, tdir > 0))
        i₋ = i₊ > 1 ? i₊ - 1 : i₊
    else
...
function sde_interpolation!(vals, tvals, id, idxs, deriv, p, continuity::Symbol = :left)
    ...
    @inbounds for j in idx
        ...
        if continuity === :left
            ...
            # and otherwise i₋ and i₊ satisfy ts[i₋] < t ≤ ts[i₊]
            i₊ = min(
                lastindex(ts), OrdinaryDiffEqCore._searchsortedfirst(ts, t, i₊, tdir > 0))
            i₋ = i₊ > 1 ? i₊ - 1 : i₊
        else
            # we have i₋ = i₊ - 1 = 1 if t < ts[1], i₊ = i₋ = lastindex(ts) if t = ts[end],
            # and otherwise i₋ and i₊ satisfy ts[i₋] ≤ t < ts[i₊]
