function sde_determine_initdt(u0::uType, t::tType, tdir, dtmax, abstol, reltol,
    ...
    g = prob.g
    p = prob.p
    d₀ = internalnorm(ArrayInterface.aos_to_soa(u0) ./
    ...
    d₁ = internalnorm(max.(internalnorm.(f₀ .+ g₀, t), internalnorm.(f₀ .- g₀, t)) ./
                      (abstol .+ internalnorm.(u0, t) .* reltol), t)
    T0 = typeof(d₀)
    T1 = typeof(d₁)
    ...
    d₂ = internalnorm(max.(internalnorm.(f₁ .- f₀ .+ ΔgMax, t),
                           internalnorm.(f₁ .- f₀ .- ΔgMax, t)) ./
                      (abstol .+ internalnorm.(u0, t) .* reltol), t) ./ dt₀
