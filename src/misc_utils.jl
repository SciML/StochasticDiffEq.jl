...
Base.@pure function DiffCache(T, size, ::Type{Val{chunk_size}}) where {chunk_size}
    DiffCache(fill(zero(T), size...),
              fill(zero(Dual{typeof(ForwardDiff.Tag(DiffEqNLSolveTag(), T)), T, chunk_size}),
                   size...))
end

Base.@pure function DiffCache(u::AbstractArray)
    DiffCache(eltype(u), size(u), Val{ForwardDiff.pickchunksize(length(u))})
end
Base.@pure function DiffCache(u::AbstractArray, nlsolve)
    DiffCache(eltype(u), size(u), Val{get_chunksize(nlsolve)})
end
Base.@pure function DiffCache(u::AbstractArray, T::Type{Val{CS}}) where {CS}
    DiffCache(eltype(u), size(u), T)
end
...
Base.@pure function determine_chunksize(u, alg::DEAlgorithm)
