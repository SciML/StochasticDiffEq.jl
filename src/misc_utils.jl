...
Base.@pure DiffCache(u::AbstractArray,nlsolve) = DiffCache(eltype(u),length(u),Val{DiffEqDiffTools.get_chunksize(nlsolve)})
Base.@pure DiffCache{CS}(u::AbstractArray,T::Type{Val{CS}}) = DiffCache(eltype(u),length(u),T)

DiffEqDiffTools.get_du{T<:Dual}(dc::DiffCache, ::Type{T}) = dc.dual_du
DiffEqDiffTools.get_du(dc::DiffCache, T) = dc.du
