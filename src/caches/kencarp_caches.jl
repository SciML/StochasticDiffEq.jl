...
function alg_cache(alg::SKenCarp, prob, u, ΔW, ΔZ, p, rate_prototype, noise_rate_prototype,
    ...
    γ, c = tab.γ, tab.c3
    nlsolver = OrdinaryDiffEq.build_nlsolver(alg, u, uprev, p, t, dt, f, rate_prototype,
                                             uEltypeNoUnits, uBottomEltypeNoUnits,
                                             tTypeNoUnits, γ, c, Val(false))
    SKenCarpConstantCache(nlsolver, tab)
end

@cache mutable struct SKenCarpCache{uType, rateType, uNoUnitsType, N, Tab, kType, randType,
                                    rateNoiseType} <: StochasticDiffEqMutableCache
    u::uType
    uprev::uType
    fsalfirst::rateType
    z₁::uType
...
function alg_cache(alg::SKenCarp, prob, u, ΔW, ΔZ, p, rate_prototype, noise_rate_prototype,
    ...
    SKenCarpCache{typeof(u), typeof(rate_prototype), typeof(atmp), typeof(nlsolver),
                  typeof(tab), typeof(k1), typeof(chi2), typeof(g1)}(u, uprev, fsalfirst,
                                                                     z₁, z₂, z₃, z₄, k1, k2,
                                                                     k3, k4, atmp, nlsolver,
                                                                     tab, chi2, g1, g4)
end
