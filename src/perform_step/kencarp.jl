@muladd function perform_step!(integrator, cache::SKenCarpConstantCache)
    ...
    if typeof(integrator.f) <: SplitSDEFunction
        ...
        u = uprev + a41 * z₁ + a42 * z₂ + a43 * z₃ + γ * z₄ + eb1 * k1 + eb2 * k2 +
            eb3 * k3 + eb4 * k4 + integrator.W.dW .* g4 + E₂
    else
    ...
    if integrator.opts.adaptive
        if alg.ode_error_est
            ...
            if alg.smooth_est # From Shampine
                E₁ = OrdinaryDiffEq._reshape(OrdinaryDiffEq.get_W(nlsolver) \
                                             OrdinaryDiffEq._vec(tmp), axes(tmp))
            else
                E₁ = tmp
            end
        else
...
@muladd function perform_step!(integrator, cache::SKenCarpCache)
    ...
    if typeof(integrator.f) <: SplitSDEFunction
        ...
        @.. tmp = uprev + a41 * z₁ + a42 * z₂ + a43 * z₃ + ea41 * k1 + ea42 * k2 +
                  ea43 * k3 + nb043 * z₄
    else
    ...
    if typeof(integrator.f) <: SplitSDEFunction
        ...
        if is_diagonal_noise(integrator.sol.prob)
            @.. E₂ = chi2 * (g1 - g4)
            @.. u = uprev + a41 * z₁ + a42 * z₂ + a43 * z₃ + γ * z₄ + integrator.W.dW * g4 +
                    E₂
        else
            g1 .-= g4
            mul!(E₂, g1, chi2)
