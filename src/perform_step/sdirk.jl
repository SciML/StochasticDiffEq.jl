...
@muladd function perform_step!(integrator,
    ...
    if integrator.opts.adaptive
        ...
        elseif typeof(cache) <: ImplicitRKMilCache
            # gtmp3 is ggprime
            @.. dz = integrator.opts.internalnorm(dW^3, t) *
                     integrator.opts.internalnorm(gtmp3, t)^2 / 6
        end
...
function OrdinaryDiffEq.update_W!(nlsolver::OrdinaryDiffEq.AbstractNLSolver, integrator,
                                  cache::StochasticDiffEqMutableCache, dtgamma, repeat_step)
    if OrdinaryDiffEq.isnewton(nlsolver)
        OrdinaryDiffEq.calc_W!(OrdinaryDiffEq.get_W(nlsolver), integrator, nlsolver, cache,
                               dtgamma, repeat_step, true)
    end
    nothing
end
