...
@muladd function perform_step!(integrator, cache::RKMilCache)
    ...
    if integrator.opts.adaptive
        ...
        @.. tmp += integrator.opts.internalnorm(integrator.opts.delta * dt * (du2 - du1) /
                                                2, t)
...
@muladd function perform_step!(integrator, cache::RKMilCommuteConstantCache)
    ...
    if is_diagonal_noise(integrator.sol.prob)
        tmp = (alg_interpretation(integrator.alg) == :Ito ? K : uprev) .+
              integrator.sqdt .* L
...
@muladd function perform_step!(integrator, cache::RKMilGeneralConstantCache)
    ...
    J = get_iterated_I(dt, dW, W.dZ, Jalg, integrator.alg.p, integrator.alg.c,
                       alg_order(integrator.alg))
    ...
    if typeof(dW) <: Number || is_diagonal_noise(integrator.sol.prob)
        K = @.. uprev + dt * du₁
        utilde = (alg_interpretation(integrator.alg) == :Ito ? K : uprev) +
                 L * integrator.sqdt
        ggprime = (integrator.g(utilde, p, t) .- L) ./ (integrator.sqdt)
...
@muladd function perform_step!(integrator, cache::RKMilGeneralCache)
    ...
    get_iterated_I!(dt, dW, W.dZ, Jalg, integrator.alg.p, integrator.alg.c,
                    alg_order(integrator.alg))
    J = Jalg.J
    ...
    if integrator.opts.adaptive
        ...
        @.. tmp = integrator.opts.internalnorm(integrator.opts.delta * dt * (du₂ - du₁) / 2,
                                               t) + En
