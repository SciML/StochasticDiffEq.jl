@muladd function perform_step!(integrator,cache::EMConstantCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::EMCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::EulerHeunConstantCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::EulerHeunCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::RandomEMConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
  u = uprev .+ dt .* integrator.f(uprev,p,t,W.curW)
  integrator.u = u
end

@muladd function perform_step!(integrator,cache::RandomEMCache,f::F=integrator.f) where F
  @unpack rtmp = cache
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::SimplifiedEMConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::SimplifiedEMCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::RKMilConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
#=
@muladd function perform_step!(integrator,cache::RKMilCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::RKMilCommuteConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::RKMilCommuteCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::RKMilGeneralConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
  Wik = cache.WikJ
  dW = W.dW
...
@muladd function perform_step!(integrator,cache::RKMilGeneralCache,f::F=integrator.f) where F
