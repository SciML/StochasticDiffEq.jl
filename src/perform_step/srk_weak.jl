@muladd function perform_step!(integrator, cache::DRI1ConstantCache)
    ...
    if typeof(W.dW) <: Number
        ...
        for k in 1:m
            if is_diagonal_noise(integrator.sol.prob)
                ...
                H22[k] += (b221 * g1[:, l] + b222 * g2[l][:, l] + b223 * g3[l][:, l]) *
                          integrator.sqdt
                H23[k] += (b231 * g1[:, l] + b232 * g2[l][:, l] + b233 * g3[l][:, l]) *
                          integrator.sqdt
            end
        end
    end
    ...
    if integrator.opts.adaptive &&
       (typeof(W.dW) <: Number || is_diagonal_noise(integrator.sol.prob) || m == 1)
...
@muladd function perform_step!(integrator, cache::RSConstantCache)
    ...
    if typeof(W.dW) <: Number
        ...
        for k in 1:m
            ...
            if is_diagonal_noise(integrator.sol.prob)
                H14[k] += b141 * g1[k] * _dW[k] .+ b142 * g2[k] * _dW[k] .+
                          b143 * g3[k] * _dW[k]
                for l in 1:m
                    if l != k
    ...
    if typeof(W.dW) <: Number
        ...
        if is_diagonal_noise(integrator.sol.prob)
            ...
            for k in 1:m
                ...
                g4k = @view g4[k][:, k]
                @.. u = u +
                        (g1k * beta11 + g2k * beta12 + g3k * beta13 + g4k * beta14) * _dW[k]
                tmpg = integrator.g(H22[k], p, t)
...
@muladd function perform_step!(integrator, cache::RSCache)
    ...
    for k in 1:m
        @.. H14[k] = uprev + a141 * k1 * dt
        if is_diagonal_noise(integrator.sol.prob)
            @.. H14[k] = H14[k] + b141 * g1[k] * _dW[k] + b142 * g2[k] * _dW[k] +
                         b143 * g3[k] * _dW[k]
            for l in 1:m
                if l != k
            ...
            @.. H14[k] = H14[k] + b141 * g1k * _dW[k] + b142 * g2k * _dW[k] +
                         b143 * g3k * _dW[k]
            for l in 1:m
                if l != k
                    g1l = @view g1[:, l]
    ...
    if is_diagonal_noise(integrator.sol.prob)
        ...
        for k in 1:m
            u[k] = u[k] +
                   (g2[k][k] * beta12 + g3[k][k] * beta13 + g4[k][k] * beta14) * _dW[k]
            if m != 1
...
@muladd function perform_step!(integrator, cache::PL1WMConstantCache)
    ...
    if !is_diagonal_noise(integrator.sol.prob) || typeof(W.dW) <: Number
        ...
        if typeof(W.dW) <: Number
            ...
            Yp = Vector{typeof(uprev)}[uprev .+ k1 * dt .+ g1[:, k] * integrator.sqdt
                                       for k in 1:m]
            Ym = Vector{typeof(uprev)}[uprev .+ k1 * dt .- g1[:, k] * integrator.sqdt
                                       for k in 1:m]
        end
    else
        Y = uprev + k1 * dt + g1 .* _dW
                ...
                for l in 1:m
                    if l != k
                        ...
                        @.. u += 1 // 4 * (tmpg1[k] - tmpg2[k]) *
                                 (_dW[k] * _dW[l] + ihat2) / integrator.sqdt
                    end
                end
            end
        else
...
@muladd function perform_step!(integrator, cache::PL1WMCache)
    ...
    if typeof(W.dW) <: Number
        ...
        @.. u = u + 1 // 4 * (tmpg1 + tmpg2 + 2 * g1) * _dW +
                1 // 4 * (tmpg1 - tmpg2) * chi1[k] / integrator.sqdt #(1.1)
    else
        if !is_diagonal_noise(integrator.sol.prob) || typeof(W.dW) <: Number
            # non-diag noise
            for k in 1:m
                ...
                for l in 1:m
                    if l != k
                        ...
                        u += 1 // 4 * (tmpg1k - tmpg2k) * (_dW[k] * _dW[l] + ihat2) /
                             integrator.sqdt
                    end
                end
            end
            ...
            for k in 1:m
                ...
                for l in 1:m
                    if l != k
                        ...
                        @.. u = u + 1 // 4 * (tmpg1[k] + tmpg2[k] - 2 * g1[k]) * _dW[k]
                        @.. u = u +
                                1 // 4 * (tmpg1[k] - tmpg2[k]) * (_dW[k] * _dW[l] + ihat2) /
                                integrator.sqdt
                    end
                end
            end
...
@muladd function perform_step!(integrator, cache::NONConstantCache)
    ...
    if typeof(W.dW) <: Number
        Y400 += a0j41 * Y1jajb
        Y4jajb = integrator.g(uprev + ajj41 * Y1jajb[1] + ajj42 * Y2jajb[1] +
                              ajj43 * Y3jajb[1] + aj041 * Y100, p, t) * _dW
    else
        if !is_diagonal_noise(integrator.sol.prob)
            for ja in 1:m
...
@muladd function perform_step!(integrator, cache::NONCache)
    ...
    if typeof(W.dW) <: Number
        ...
        if is_diagonal_noise(integrator.sol.prob)
            for ja in 1:m
                @.. u = u + cj1 * Y1jajb[ja, ja] + cj2 * Y2jajb[ja, ja] +
                        cj3 * Y3jajb[ja, ja] + cj4 * Y4jajb[ja, ja]
                for jb in 1:m
                    if jb != ja
                        if ja > jb
...
@muladd function perform_step!(integrator, cache::COMConstantCache)
    ...
    if typeof(W.dW) <: Number
        ...
        if !is_diagonal_noise(integrator.sol.prob)
            ...
            for j in 1:m
                tmpu = copy(tmpu2)
                tmpu[j] += ajj31 * Y1j[j] + ajj32 * Y2j[j] -
                           (ajl31 * Y1j[j] + ajl32 * Y2j[j])
                Y3j[j] = integrator.g(tmpu, p, t)[j] * _dW[j]
            end
        end
    end
    ...
    if typeof(W.dW) <: Number
        Y40 += a0j41 * Y1j
        Y4j = integrator.g(uprev + ajj41 * Y1j + ajj42 * Y2j + ajj43 * Y3j + aj041 * Y10, p,
                           t) * _dW
    else
        if !is_diagonal_noise(integrator.sol.prob)
    ...
    if typeof(W.dW) <: Number
        ...
        if is_diagonal_noise(integrator.sol.prob)
            ...
            for j in 1:m
                u += @. cj1 * Y1j[:, j] + cj2 * Y2j[:, j] + cj3 * Y3j[:, j] +
                        cj4 * Y4j[:, j]
            end
        end
    end
...
@muladd function perform_step!(integrator, cache::SIESMEConstantCache)
    ...
    if is_diagonal_noise(integrator.sol.prob)
        ...
        g1 = integrator.g(uprev + λbar0 * k0 * dt + β2 * g0 * integrator.sqdt +
                          β3 * g0 .* W2, p, t + µbar0 * dt)
        g2 = integrator.g(uprev + λbar0 * k0 * dt + δ2 * g0 * integrator.sqdt +
                          δ3 * g0 .* W2, p, t + µbar0 * dt)
