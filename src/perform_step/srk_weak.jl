...
@muladd function perform_step!(integrator, cache::DRI1Cache)
    ...
    if W.dW isa Union{SArray, Number}
        # tbd
        _dW = map(
            x -> calc_threepoint_random(sq3dt, NORMAL_ONESIX_QUANTILE, x), W.dW / sqrt(dt))
    else
        # define three-point distributed random variables
        sqrtdt = sqrt(dt)
        @.. chi1 = W.dW / sqrtdt
...
@muladd function perform_step!(integrator, cache::NONConstantCache)
    ...
    if W.dW isa Number
        Y400 += a0j41 * Y1jajb
        Y4jajb = integrator.g(
            uprev + ajj41 * Y1jajb[1] + ajj42 * Y2jajb[1] + ajj43 * Y3jajb[1] +
            aj041 * Y100,
            p,
            t) * _dW
    else
...
@muladd function perform_step!(integrator, cache::COMConstantCache)
    ...
    if W.dW isa Number
        Y40 += a0j41 * Y1j
        Y4j = integrator.g(
            uprev + ajj41 * Y1j + ajj42 * Y2j + ajj43 * Y3j + aj041 * Y10, p, t) * _dW
    else
        if !is_diagonal_noise(integrator.sol.prob)
...
@muladd function perform_step!(integrator, cache::SIESMEConstantCache)
    ...
    if is_diagonal_noise(integrator.sol.prob)
        ...
        g1 = integrator.g(
            uprev + λbar0 * k0 * dt + β2 * g0 * integrator.sqdt + β3 * g0 .* W2,
            p, t + µbar0 * dt)
        g2 = integrator.g(
            uprev + λbar0 * k0 * dt + δ2 * g0 * integrator.sqdt + δ3 * g0 .* W2,
            p, t + µbar0 * dt)
