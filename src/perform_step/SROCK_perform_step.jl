@muladd function perform_step!(integrator, cache::SROCK1ConstantCache)
    ...
    cache.mdeg = Int(floor(sqrt(2 * dt *
                                integrator.opts.internalnorm(integrator.eigen_est, t)) + 1)) # this is the spectral radius estimate to choose optimal stage
...
@muladd function perform_step!(integrator, cache::SROCK1Cache)
    ...
    alg = unwrap_alg(integrator, true)
    alg.eigen_est === nothing ? maxeig!(integrator, cache) : alg.eigen_est(integrator)
    ccache.mdeg = Int(floor(sqrt(2 * dt *
                                 integrator.opts.internalnorm(integrator.eigen_est, t)) + 1))   # this is the spectral radius estimate to choose optimal stage
    choose_deg!(integrator, cache)
...
@muladd function perform_step!(integrator, cache::SROCK2ConstantCache)
    ...
    gen_prob = !((is_diagonal_noise(integrator.sol.prob)) || (typeof(W.dW) <: Number) ||
                 (length(W.dW) == 1))
    gen_prob && (vec_χ = 2 .*
    ...
    alg.eigen_est === nothing ? maxeig!(integrator, cache) : alg.eigen_est(integrator)
    cache.mdeg = Int(floor(sqrt((2 * dt *
                                 integrator.opts.internalnorm(integrator.eigen_est, t) +
                                 1.5) / 0.811) + 1))
...
@muladd function perform_step!(integrator, cache::SKSROCKConstantCache)
    ...
    alg.eigen_est === nothing ? maxeig!(integrator, cache) : alg.eigen_est(integrator)
    η = oftype(t, 0.05)
    mdeg = Int(floor(sqrt((dt * integrator.opts.internalnorm(integrator.eigen_est, t) + 1.5) /
                          (2 - η * 4 / 3)) + 1))
...
@muladd function perform_step!(integrator, cache::TangXiaoSROCK2ConstantCache)
    ...
    alg = unwrap_alg(integrator, true)
    alg.eigen_est === nothing ? maxeig!(integrator, cache) : alg.eigen_est(integrator)
    (integrator.alg.version_num <= 2) && (cache.mdeg = Int(floor(sqrt((dt *
                                  integrator.opts.internalnorm(integrator.eigen_est, t) + 1.5) / 0.811) + 1)))
    (integrator.alg.version_num > 2) && (cache.mdeg = Int(floor(sqrt((dt *
                                  integrator.opts.internalnorm(integrator.eigen_est, t) + 1.5) / 0.611) + 1)))
    ...
    for i in 0:(mdeg + 1)
        if i == 1
            ...
            uᵢ₋₁ = uprev + α * dt * μ * uᵢ
        elseif i > 1 && i <= mdeg
            μ, ν, κ = recf[start + 2 * (i - 2) + 1], 1 + recf[start + 2 * (i - 2) + 2],
                      recf[start + 2 * (i - 2) + 2]
            ...
            tᵢ = α * dt * μ + ν * tᵢ₋₁ - κ * tᵢ₋₂
        elseif i == mdeg + 1
            μ, ν, κ = recf2[(deg_index - 1) * 4 + 1], 1 + recf2[(deg_index - 1) * 4 + 2],
    ...
    elseif is_diagonal_noise(integrator.sol.prob)
        ...
        for i in 1:length(W.dW)
            ...
            u += @view(Gₛ₁[:, i]) * W.dW[i] +
                 (@view(Gₛ[:, i]) - @view(Gₛ₁[:, i])) * ((W.dW[i]^2 - dt) / (η₁ * sqrt_dt))
        end
...
@muladd function perform_step!(integrator, cache::TangXiaoSROCK2Cache)
    ...
    alg = unwrap_alg(integrator, true)
    alg.eigen_est === nothing ? maxeig!(integrator, cache) : alg.eigen_est(integrator)
    (integrator.alg.version_num <= 2) && (ccache.mdeg = Int(floor(sqrt((dt *
                                   integrator.opts.internalnorm(integrator.eigen_est, t) + 1.5) / 0.811) + 1)))
    (integrator.alg.version_num > 2) && (ccache.mdeg = Int(floor(sqrt((dt *
                                   integrator.opts.internalnorm(integrator.eigen_est, t) + 1.5) / 0.611) + 1)))
...
@muladd function perform_step!(integrator, cache::KomBurSROCK2ConstantCache)
    ...
    if typeof(W.dW) <: Number || length(W.dW) == 1 || is_diagonal_noise(integrator.sol.prob)
        ...
        utmp = uᵢ₋₁ + μₛ₋₃ * yₛ₋₃ + δ₁ * yₛ₋₂ + 1 // 12 .* W.dW .* Xₛ₋₃ +
               1 // 4 .* W.dW .* Xₛ₋₂
        ttmp = tᵢ₋₁ + μₛ₋₃ + δ₁
        Xₛ₋₁ = integrator.g(utmp, p, ttmp)
        ...
        #stage s
        utmp = uᵢ₋₂ + C₁ * yₛ₋₃ + μₛ₋₂ * yₛ₋₂ + σ * dt * yₛ₋₁ + δ₆ * SXₛ₋₃ + δ₇ * SXₛ₋₂ +
               δ₈ * SXₛ₋₁
        ttmp = tᵢ₋₂ + C₁ + μₛ₋₂ + σ * dt
        utmp = integrator.f(utmp, p, ttmp)
        u += (σ + τ) * dt * utmp
        ...
        for i in 1:length(W.dW)
            ...
            utmp = uᵢ₋₁ + μₛ₋₃ * yₛ₋₃ + δ₂ * yₛ₋₂ + δ₃ * yₛ₋₁ + 1 // 4 * SXₛ₋₃ +
                   3 // 4 * SXₛ₋₂ - 3 // 2 * (Xₛ₋₃ * WikRange) -
                   1 // 2 * (Xₛ₋₂ * WikRange) + 2 * (Xₛ₋₁ * WikRange)
            ttmp = tᵢ₋₁ + μₛ₋₃ + δ₂ + δ₃
            Gₛ = integrator.g(utmp, p, ttmp)
        ...
        for i in 1:length(W.dW)
            ...
            for j in 1:length(W.dW)
                ...
                if j != i
                    ...
                    SXₛ₋₁ += (i > j ? -vec_χ[i] * W.dW[j] : vec_χ[j] * W.dW[i]) *
                             (Xₛ₋₃ * WikRange)
                end
            end
            ttmp = tᵢ₋₁ + μₛ₋₃
...
@muladd function perform_step!(integrator, cache::KomBurSROCK2Cache)
    ...
    sqrt_dt = sqrt(dt)
    (gen_prob) &&
        (vec_χ .= 2 .* floor.(1 // 2 .+ false .* vec_χ .+ rand(length(vec_χ))) .- 1)
    ...
    if typeof(W.dW) <: Number || length(W.dW) == 1 || is_diagonal_noise(integrator.sol.prob)
        ...
        @.. uᵢ₋₁ = uᵢ₋₁ + μₛ₋₃ * yₛ₋₃ + δ₂ * yₛ₋₂ + δ₃ * yₛ₋₁ - 5 // 4 * W.dW * Xₛ₋₃ +
                   1 // 4 * W.dW * Xₛ₋₂ + 2 * W.dW * Xₛ₋₁
        tᵢ₋₁ = tᵢ₋₁ + μₛ₋₃ + δ₂ + δ₃
        ...
        #stage s
        @.. utmp = uᵢ₋₂ + C₁ * yₛ₋₃ + μₛ₋₂ * yₛ₋₂ + σ * dt * yₛ₋₁ + δ₆ * SXₛ₋₃ +
                   δ₇ * SXₛ₋₂ + δ₈ * SXₛ₋₁
        ttmp = tᵢ₋₂ + C₁ + μₛ₋₂ + σ * dt
        # utmp = integrator.f(utmp,p,ttmp)
        # u += (τ/σ)*dt*utmp
