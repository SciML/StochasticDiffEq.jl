@muladd function perform_step!(integrator,cache::SROCK1ConstantCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::SROCK1Cache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::SROCK2ConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::SROCK2Cache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::SROCKEMConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::SROCKEMCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::SKSROCKConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::SKSROCKCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::TangXiaoSROCK2ConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
  ...
  for i in 0:mdeg+1
    if i == 1
      ...
      uᵢ₋₁ = uprev + α*dt*μ*uᵢ
    elseif i > 1 && i <= mdeg
      μ, ν, κ = recf[start + 2*(i-2) + 1], 1 + recf[start + 2*(i-2) + 2], recf[start + 2*(i-2) + 2]
...
@muladd function perform_step!(integrator,cache::TangXiaoSROCK2Cache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::KomBurSROCK2ConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::KomBurSROCK2Cache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::SROCKC2ConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::SROCKC2Cache,f::F=integrator.f) where F
