@muladd function perform_step!(integrator,cache::SROCK_1ConstantCache,f=integrator.f)
  ...
  mdeg = Int(floor(sqrt(2*dt*integrator.eigen_est/0.66)+1))
  cache.mdeg = max(5,min(mdeg,250))
  ...
  for i in 2:mdeg
    ...
    (i == mdeg - 1) && (gₘ₋₂ = integrator.g(uᵢ₋₁,p,t); u += α*W.dW*gₘ₋₂)
    # (i == mdeg) && (gₘ₋₁ = integrator.g(uᵢ₋₁,p,tᵢ₋₁); u += β*W.dW*gₘ₋₂ + γ*W.dW*gₘ₋₁)
    (i == mdeg) && (gₘ₋₁ = integrator.g(uᵢ₋₁,p,t); u += β*W.dW*gₘ₋₂ + γ*W.dW*gₘ₋₁)
...
@muladd function perform_step!(integrator,cache::SROCK_1Cache,f=integrator.f)
  ...
  ccache.mdeg = max(5,min(mdeg,250))
  choose_deg!(integrator,cache)
  uᵢ₋₂ = cache.tmp
  mdeg = ccache.mdeg
  η  = ccache.optimal_η
