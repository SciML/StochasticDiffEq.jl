@muladd function perform_step!(integrator, cache::SROCK1ConstantCache)
    ...
    cache.mdeg = Int(floor(sqrt(2 * abs(dt) *
...
@muladd function perform_step!(integrator, cache::SROCK1Cache)
    ...
    alg = unwrap_alg(integrator, true)
    alg.eigen_est === nothing ? maxeig!(integrator, cache) : alg.eigen_est(integrator)
    ccache.mdeg = Int(floor(sqrt(2 * abs(dt) *
                                 integrator.opts.internalnorm(integrator.eigen_est, t)) +
                            1))   # this is the spectral radius estimate to choose optimal stage
    choose_deg!(integrator, cache)
...
@muladd function perform_step!(integrator, cache::SROCK2ConstantCache)
    ...
    gen_prob = !((is_diagonal_noise(integrator.sol.prob)) || (W.dW isa Number) ||
                 (length(W.dW) == 1))
    gen_prob && (vec_χ = 2 .*
             floor.(false .* W.dW .+ 1 // 2 .+ oftype(W.dW, rand(W.rng, length(W.dW)))) .- true)
...
@muladd function perform_step!(integrator, cache::SROCKEMConstantCache)
    ...
    if integrator.alg.strong_order_1
        cache.mdeg = Int(floor(sqrt(abs(dt) *
                                    integrator.opts.internalnorm(integrator.eigen_est, t) /
                                    0.19) + 1))
    else
        cache.mdeg = Int(floor(sqrt(abs(dt) *
...
@muladd function perform_step!(integrator, cache::SROCKEMCache)
    ...
    alg.eigen_est === nothing ? maxeig!(integrator, cache) : alg.eigen_est(integrator)
    if integrator.alg.strong_order_1
        ccache.mdeg = Int(floor(sqrt(abs(dt) *
                                     integrator.opts.internalnorm(integrator.eigen_est, t) /
                                     0.19) + 1))
    else
        ccache.mdeg = Int(floor(sqrt(abs(dt) *
...
@muladd function perform_step!(integrator, cache::SKSROCKConstantCache)
    ...
    alg.eigen_est === nothing ? maxeig!(integrator, cache) : alg.eigen_est(integrator)
    η = oftype(t, 0.05)
    mdeg = Int(floor(sqrt((abs(dt) * integrator.opts.internalnorm(integrator.eigen_est, t) +
                           1.5) / (2 - η * 4 / 3)) + 1))
...
@muladd function perform_step!(integrator, cache::TangXiaoSROCK2ConstantCache)
    ...
    (integrator.alg.version_num <= 2) &&
        (cache.mdeg = Int(floor(sqrt((abs(dt) * integrator.opts.internalnorm(
            integrator.eigen_est, t) + 1.5) / 0.811) + 1)))
    (integrator.alg.version_num > 2) &&
        (cache.mdeg = Int(floor(sqrt((abs(dt) * integrator.opts.internalnorm(
            integrator.eigen_est, t) + 1.5) / 0.611) + 1)))
...
@muladd function perform_step!(integrator, cache::TangXiaoSROCK2Cache)
    ...
    alg.eigen_est === nothing ? maxeig!(integrator, cache) : alg.eigen_est(integrator)
    (integrator.alg.version_num <= 2) &&
        (ccache.mdeg = Int(floor(sqrt((abs(dt) * integrator.opts.internalnorm(
            integrator.eigen_est, t) + 1.5) / 0.811) + 1)))
    (integrator.alg.version_num > 2) &&
        (ccache.mdeg = Int(floor(sqrt((abs(dt) * integrator.opts.internalnorm(
            integrator.eigen_est, t) + 1.5) / 0.611) + 1)))
