@muladd function perform_step!(integrator,cache::LambaEMConstantCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::LambaEMCache,f::F=integrator.f) where F
...
@muladd function perform_step!(integrator,cache::LambaEulerHeunConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
  du1 = integrator.f(uprev,p,t)
  K = uprev + dt * du1
...
@muladd function perform_step!(integrator,cache::LambaEulerHeunCache,f::F=integrator.f) where F
  @unpack du1,du2,K,tmp,L,gtmp,dW_cache = cache
  @unpack t,dt,uprev,u,W,p = integrator
  integrator.f(du1,uprev,p,t)
