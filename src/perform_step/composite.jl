    ...
    if new_current != cache.current
        ...
        if cache.current == 1 && new_current == 2
            reset_alg_dependent_opts!(
                integrator, integrator.alg.algs[1], integrator.alg.algs[2])
            transfer_cache!(
                integrator, integrator.cache.caches[1], integrator.cache.caches[2])
        elseif cache.current == 2 && new_current == 1
            reset_alg_dependent_opts!(
                integrator, integrator.alg.algs[2], integrator.alg.algs[1])
            transfer_cache!(
                integrator, integrator.cache.caches[2], integrator.cache.caches[1])
        else
    ...
    if integrator.opts.controller isa PIController
        ...
        if integrator.opts.controller.beta1 ==
           beta1_default(alg1, integrator.opts.controller.beta2)
            integrator.opts.controller.beta1 = beta1_default(
                alg2, integrator.opts.controller.beta2)
        end
    end
end
