#=
@muladd function perform_step!(integrator,cache::SRICache,f::F=integrator.f) where F
...
#=
@muladd function perform_step!(integrator,cache::SRIW1Cache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::SRIW1ConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
  sqrt3 = sqrt(3one(eltype(W.dW)))
  chi1 = @.. (W.dW.^2 - dt)/2integrator.sqdt #I_(1,1)/sqrt(h)
  chi2 = @.. (W.dW + W.dZ/sqrt3)/2 #I_(1,0)/h
...
@muladd function perform_step!(integrator,cache::SRIConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::FourStageSRIConstantCache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
...
@muladd function perform_step!(integrator,cache::FourStageSRICache,f::F=integrator.f) where F
  @unpack t,dt,uprev,u,W,p = integrator
