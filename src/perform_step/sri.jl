...
@muladd function perform_step!(integrator, cache::SRIConstantCache)
    ...
    sqrt3 = sqrt(3one(eltype(W.dW)))
    chi1 = 0.5 * (W.dW .^ 2 - dt) / integrator.sqdt #I_(1,1)/sqrt(h)
...
@muladd function perform_step!(integrator, cache::FourStageSRIConstantCache)
    ...
    H03 = uprev + dt * (a041 * k1 + a042 * k2 + a043 * k3) +
    ...
    if typeof(integrator.alg) <: StochasticCompositeAlgorithm &&
       typeof(integrator.alg.algs[1]) <: SOSRI2
...
@muladd function perform_step!(integrator, cache::FourStageSRICache)
    ...
    @.. tmp = uprev + dt * (a141 * k1 + a142 * k2 + a143 * k3) +
              sqdt * (b141 * g1 + b142 * g2 + b143 * g3)
