...
((-2θ + 3θ^2) + (6θ*(1-θ)/c2)*γ)
=#
function SKenCarpTableau(::Type{T},::Type{T2}) where {T<:CompiledFloats,T2<:CompiledFloats}
  γ  = convert(T,0.435866521508459)
  a31 = convert(T,0.2576482460664272)
  a32 = -convert(T,0.09351476757488625)
  a41 = convert(T,0.18764102434672383)
...
function SKenCarpTableau(T,T2)
  ...
  nb021 = convert(T,parse(BigFloat,"-12.246764387585055918338744103409192607986567514699471403397969732723452087723101"))
  nb043 = convert(T,parse(BigFloat,"-14.432096958608752822047165680776748797565142459789556194474191884258734697161106"))
  SKenCarpTableau(γ,a31,a32,a41,a42,a43,btilde1,btilde2,btilde3,btilde4,c3,α31,
                  α32,α41,α42,ea21,ea31,ea32,ea41,ea42,ea43,eb1,eb2,eb3,eb4,
                  ebtilde1,ebtilde2,ebtilde3,ebtilde4,nb021,nb043)
end
