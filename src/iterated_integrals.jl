...
function get_iterated_I!(dt, dW, dZ, alg::IteratedIntegralAlgorithm_iip, p = nothing, c = 1,
                         γ = 1 // 1)
    ...
    if alg.ii_approx isa IILevyArea
        if isinplace(prob)
            if typeof(ΔW) <: Number || is_diagonal_noise(prob)
                ...
                return IteratedIntegralAlgorithm_iip(ΔW,
                                                     LevyArea.optimal_algorithm(length(ΔW),
                                                                                dt))
            end
        else
            if typeof(ΔW) <: Number || is_diagonal_noise(prob)
