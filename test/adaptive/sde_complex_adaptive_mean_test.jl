...
let
    ...
    for i in 1:Ntraj
        ...
        scb1 = SavingCallback(fout, out1, saveat = T, save_everystep = false,
                              save_start = false)

        solve(prob1, RKMil{:Stratonovich}(), dt = 1e-4, callback = scb1, seed = i,
              adaptive = false)
...
let
    ...
    for i in 1:Ntraj
        ...
        solve(prob1, RKMilGeneral(interpretation = :Stratonovich), dt = 1e-4,
        ...
        solve(prob1, RKMilGeneral(interpretation = :Stratonovich), tstops = T,
              callback = scb1,
              save_everystep = false, save_start = false)
        avg2 .+= out1.saveval ./ Ntraj
        ...
        solve(prob2, RKMilGeneral(interpretation = :Stratonovich), tstops = T,
              callback = scb1,
              save_everystep = false, save_start = false)
        avg3 .+= out1.saveval ./ Ntraj
    end
