...
function weak_error(prob, alg, numtraj, batchsize, f_true, trange; abstol = 1, reltol = 0,
...
function f1!(du, u, p, t)
    @inbounds begin du[1] = 1 // 2 * u[1] + sqrt(u[1]^2 + 1) end
    nothing
end

function g1!(du, u, p, t)
    @inbounds begin du[1] = sqrt(u[1]^2 + 1) end
    nothing
end
...
for i in 1:2
    ...
    err1 = weak_error(probs[i], DRI1(), numtraj, Int(10), ftrue[i], tsave, abstol = 1.0f0,
                      reltol = 1.0f0)
    @show err1
    err2 = weak_error(probs[i], DRI1(), numtraj, Int(10), ftrue[i], tsave, abstol = 0.5f0,
                      reltol = 0.5f0)
    @show err2
    err3 = weak_error(probs[i], DRI1(), numtraj, Int(10), ftrue[i], tsave, abstol = 0.1f0,
                      reltol = 0.1f0)
    @show err3
    @test err1 > err2
    @test err2 > err3
...
for i in 1:2
    @show i
    err1 = weak_error(probs[i], DRI1NM(), numtraj, Int(1e1), ftrue[i], tsave,
                      abstol = 1.0f0, reltol = 1.0f0)
    @show err1
    err2 = weak_error(probs[i], DRI1NM(), numtraj, Int(1e1), ftrue[i], tsave,
                      abstol = 0.1f0, reltol = 0.1f0)
    @show err2
