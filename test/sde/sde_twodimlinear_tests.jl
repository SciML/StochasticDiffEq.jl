...
sol = solve(prob, ImplicitEM(nlsolve = StochasticDiffEq.NLFunctional()),
    dt = 1 / 2^(3), adaptive = false)
...
sol2 = solve(
    prob, SRI(tableau = StochasticDiffEq.constructSRIOpt1()), dt = 1 / 2^(3), seed = 1)
sol.t ≈ sol2.t
...
sim = test_convergence(
    dts, prob, ImplicitEM(symplectic = true, autodiff = false), trajectories = 100)
@test abs(sim.𝒪est[:l2] - 0.5) < 0.1
...
sim = test_convergence(
    dts, prob, ImplicitRKMil(theta = 1, autodiff = false), trajectories = 200)
@test abs(sim.𝒪est[:l2] - 1) < 0.1
...
sim = test_convergence(
    dts, prob, ImplicitRKMil(symplectic = true, autodiff = false), trajectories = 100)
@test abs(sim.𝒪est[:l2] - 1) < 0.1

sim = test_convergence(
    dts, prob, ImplicitRKMil(nlsolve = StochasticDiffEq.NLFunctional()), trajectories = 100)
@test abs(sim.𝒪est[:l2] - 1) < 0.1
