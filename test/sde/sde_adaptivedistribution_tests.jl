...
for j = 1:M
  ...
  for i = 1:N
    sol =solve(prob::SDEProblem,[0,T],dt=1/2^(4),save_timeseries=true,alg=:SRI,adaptive=true,abstol=1e-2,reltol=0,adaptivealg=:RSwM1)
    Wends[i] = sol.Ws[end]
  end
...
for j = 1:M
  Wends = Vector{Float64}(N)
  for i = 1:N
    sol =solve(prob::SDEProblem,[0,T],dt=1/2^(4),save_timeseries=true,alg=:SRI,adaptive=true,abstol=1e-2,reltol=0,adaptivealg=:RSwM2)
    Wends[i] = sol.Ws[end]
  end
  kssol = ApproximateOneSampleKSTest(Wends/sqrt(T), Normal())
  ps[j] = pvalue(kssol) #Should be not significant (most of the time)
...
for j = 1:M
  Wends = Vector{Float64}(N)
  for i = 1:N
    sol =solve(prob::SDEProblem,[0,T],dt=1/2^(4),save_timeseries=true,alg=:SRI,adaptive=true,abstol=1e-2,reltol=0,adaptivealg=:RSwM3)
    Wends[i] = sol.Ws[end]
  end
  kssol = ApproximateOneSampleKSTest(Wends/sqrt(T), Normal())
  ps[j] = pvalue(kssol) #Should be not significant (most of the time)
