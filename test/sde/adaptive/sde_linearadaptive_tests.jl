...
sol =solve(prob,[0,1],dt=1//2^(4),save_timeseries=true,alg=:SRI,adaptive=true,abstol=1,reltol=0)
err1 = sol.errors[:final]
println("Final error for the first solution was $err1")
TEST_PLOT && p1 = plot(sol,plot_analytic=true)

sol2 =solve(prob,[0,1],dt=1//2^(4),save_timeseries=true,alg=:SRI,adaptive=true,abstol=1e-1,reltol=0)
err2 = sol2.errors[:final]
println("Final error for the second solution was $err2")
TEST_PLOT && p2 = plot(sol2,plot_analytic=true)

sol3 =solve(prob,[0,1],dt=BigInt(1)//BigInt(2)^(4),save_timeseries=true,alg=:SRI,adaptive=true,abstol=1e-2,reltol=0)
err3 = sol3.errors[:final]
println("Final error for the third solution was $err3")
TEST_PLOT && p3 = plot(sol3,plot_analytic=true)

sol4 =solve(prob,[0,1],dt=1/2^(4),save_timeseries=true,alg=:SRI,adaptive=true,abstol=1e-3,reltol=0)
