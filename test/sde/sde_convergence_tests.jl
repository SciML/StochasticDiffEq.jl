...
sim = test_convergence(
    dts, prob, ISSEM(nlsolve = StochasticDiffEq.NLFunctional()), trajectories = Int(1e2))
@test abs(sim.𝒪est[:l2] - 0.5) < 0.2
...
@time sim2 = test_convergence(
    dts, prob, SROCKEM(strong_order_1 = false), trajectories = Int(1e2))
@test abs(sim2.𝒪est[:l∞] - 1) < 0.2
...
sim10 = test_convergence(
    dts, prob, SKenCarp(nlsolve = StochasticDiffEq.NLFunctional()), trajectories = Int(1e1))
@test abs(sim10.𝒪est[:final] - 2) < 0.3

sim2 = test_convergence(
    dts, prob, SRA(tableau = StochasticDiffEq.constructSRA2()), trajectories = Int(1e1))
@test abs(sim2.𝒪est[:final] - 2) < 0.3
sim3 = test_convergence(
    dts, prob, SRA(tableau = StochasticDiffEq.constructSRA3()), trajectories = Int(1e2))
