...
sim = test_convergence(
    dts, prob, ImplicitEulerHeun(symplectic = true), trajectories = Int(5e2))
@test abs(sim.𝒪est[:l2] - 1) < 0.1
...
sim = test_convergence(
    dts, prob, ImplicitEulerHeun(symplectic = true), trajectories = Int(5e1))
@test abs(sim.𝒪est[:l2] - 1) < 0.1
...
sim = test_convergence(dts,
    prob,
    ImplicitRKMil(
        theta = 1, interpretation = SciMLBase.AlgorithmInterpretation.Stratonovich),
    trajectories = Int(1e2))
@test abs(sim.𝒪est[:l2] - 1) < 0.1
...
sim = test_convergence(dts,
    prob,
    ImplicitRKMil(
        symplectic = true, interpretation = SciMLBase.AlgorithmInterpretation.Stratonovich),
    trajectories = Int(1e2))
@test abs(sim.𝒪est[:l2] - 1) < 0.1
