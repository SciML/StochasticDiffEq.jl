...
importsdeproblems();
using DiffEqProblemLibrary.SDEProblemLibrary: prob_sde_linear_stratonovich,
...
sim = test_convergence(dts, prob, ImplicitEulerHeun(symplectic = true),
                       trajectories = Int(5e2))
@test abs(sim.𝒪est[:l2] - 1) < 0.1

sim = test_convergence(dts, prob, RKMil(interpretation = :Stratonovich),
                       trajectories = Int(5e2))
@test abs(sim.𝒪est[:l2] - 1) < 0.2

sim = test_convergence(dts, prob, RKMilGeneral(interpretation = :Stratonovich),
                       trajectories = Int(5e2))
@test abs(sim.𝒪est[:l2] - 1) < 0.2
...
sim = test_convergence(dts, prob, SROCK1(interpretation = :Stratonovich),
                       trajectories = Int(2e2))
...
sim = test_convergence(dts, prob,
                       ImplicitRKMil(symplectic = true, interpretation = :Stratonovich),
                       trajectories = Int(1e2))
@test abs(sim.𝒪est[:l2] - 1) < 0.1
