...
if hasmethod(calc_W, (Any, Any, Any, Any, Any))
    function OrdinaryDiffEqDifferentiation.calc_W(
            integ, nlsolver, dgamma, repeat_step::Bool)
        OrdinaryDiffEqDifferentiation.calc_W(integ, nlsolver, dgamma, repeat_step, true)
    end
    function OrdinaryDiffEqDifferentiation.calc_W!(
            integ, nlsolver, cache, dgamma, repeat_step::Bool)
        OrdinaryDiffEqDifferentiation.calc_W(
...
@testset "Derivative Utilities" begin
    @testset "calc_W!" begin
        ...
        W = calc_W(integrator, integrator.cache.nlsolver, dtgamma, false) #=repeat_step=#
        ...
        calc_W!(W, integrator, integrator.cache.nlsolver, integrator.cache, dtgamma, false) #=repeat_step=#
        ...
        StochasticDiffEq.OrdinaryDiffEqDifferentiation.jacobian2W!(
    ...
    @testset "Implicit solver with lazy W" begin
        ...
        prob2 = SDEProblem(
            SDEFunction(_f, _g; mass_matrix = mm, jac = (u, p, t) -> t * A), u0, tspan)
        prob1_ip = SDEProblem(SDEFunction(_f_ip, _g_ip; mass_matrix = mm), u0, tspan)
        jac_prototype = MatrixOperator(
            similar(A); update_func! = (J, u, p, t) -> (J .= t .* A; J))
        prob2_ip = SDEProblem(
            SDEFunction(_f_ip, _g_ip; mass_matrix = mm, jac_prototype = jac_prototype),
            u0, tspan)
        ...
        prob2_ip = SDEProblem(
            SDEFunction(_f_ip, _g_ip; jac_prototype = jac_prototype), u0, tspan)
