...
@testset "Derivative Utilities" begin
    @testset "calc_W!" begin
        ...
        StochasticDiffEq.OrdinaryDiffEq.jacobian2W!(integrator.cache.nlsolver.cache.W._concrete_form,
                                                    mm, dtgamma,
                                                    integrator.cache.nlsolver.cache.W.J.A,
                                                    false)
        @test convert(AbstractMatrix, integrator.cache.nlsolver.cache.W) == concrete_W
        ldiv!(tmp, lu!(integrator.cache.nlsolver.cache.W), u0)
        @test tmp == concrete_W \ u0
    ...
    @testset "Implicit solver with lazy W" begin
        ...
        prob2 = SDEProblem(SDEFunction(_f, _g; mass_matrix = mm, jac = (u, p, t) -> t * A),
                           _g, u0, tspan)
        prob1_ip = SDEProblem(SDEFunction(_f_ip, _g_ip; mass_matrix = mm), _g_ip, u0, tspan)
        jac_prototype = DiffEqArrayOperator(similar(A);
                                            update_func = (J, u, p, t) -> (J .= t .* A; J))
        prob2_ip = SDEProblem(SDEFunction(_f_ip, _g_ip; mass_matrix = mm,
                                          jac_prototype = jac_prototype), _g_ip, u0, tspan)
        ...
        prob2_ip = SDEProblem(SDEFunction(_f_ip, _g_ip; jac_prototype = jac_prototype),
                              _g_ip, u0, tspan)
