...
prob = SDEProblem(ff_noncommute, g_noncommute, u0, (0.0, 1.0),
                  noise_rate_prototype = rand(4, m))
...
ff_noncommute_stratonovich = SDEFunction(f_noncommute, g_noncommute,
                                         analytic = f_noncommute_analytic_stratonovich)
prob_stratonovich = SDEProblem(ff_noncommute_stratonovich, g_noncommute, u0, (0.0, 1.0),
                               noise_rate_prototype = rand(4, m))
...
p = [α]
prob2 = SDEProblem(f_noncommute_2, g_noncommute_2, u0, (0.0, 0.5), p,
                   noise_rate_prototype = rand(2, m))

sol1 = solve(prob2, EM(), dt = 1 / 2^(8))
sol2 = solve(prob2, RKMilGeneral(p = true, dt = 1 / 2^(7)), dt = 1 / 2^(7),
             adaptive = false)
sol3 = solve(prob2, RKMilGeneral(), dt = 1 / 2^(7))

dts = (1 / 2) .^ (6:-1:2) #14->7 good plot
test_dt = 1 / 2^(10)
sim5 = analyticless_test_convergence(dts, prob2, EM(), test_dt, trajectories = 300,
                                     use_noise_grid = false)
@test abs(sim5.𝒪est[:final] - 0.5) < 0.2
sim6 = analyticless_test_convergence(dts, prob2, RKMilGeneral(p = true, dt = test_dt),
