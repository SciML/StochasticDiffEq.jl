...
for i in 1:2
    ...
    err1 = weak_error(probs[i], DRI1NM(), numtraj, Int(1e4), ftrue[i], tsave,
                      abstol = 1.0f0, reltol = 1.0f0, ensemblealg = EnsembleGPUArray())
    @show err1
    # err2 = weak_error(probs[i],DRI1NM(),numtraj,Int(1e4),ftrue[i],tsave,abstol=0.1f0,reltol=0.1f0, ensemblealg=EnsembleGPUArray())
    # @show err2
    err3 = weak_error(probs[i], DRI1NM(), numtraj, Int(1e4), ftrue[i], tsave,
                      abstol = 0.01f0, reltol = 0.01f0, ensemblealg = EnsembleGPUArray())
    @show err3
    @test err1 > err3
    println("")
end
