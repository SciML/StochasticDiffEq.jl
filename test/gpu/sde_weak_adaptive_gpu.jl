...
function weak_error(prob, alg, numtraj, batchsize, f_true, trange;
...
for i in 1:2
    ...
    err1 = weak_error(
        probs[i], DRI1NM(), numtraj, Int(1e4), ftrue[i], tsave, abstol = 1.0f0,
    ...
    # @show err2
    err3 = weak_error(
        probs[i], DRI1NM(), numtraj, Int(1e4), ftrue[i], tsave, abstol = 0.01f0,
