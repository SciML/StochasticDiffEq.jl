...
Random.seed!(200)
sol2 = solve(prob2, EM(), dt = 1 // 2^(4), saveat = 0.33,
...
sol = solve(prob2, EM(), dt = 1 // 2^(4), saveat = 0.33,
    save_on = false, save_start = false, save_end = false)
@test isempty(sol.t) && isempty(sol.u)

_saveat = [0.0, 0.25, 0.5, 1.0]
integ = init(
    SDEProblem((u, p, t) -> u, (u, p, t) -> u, 0.0, (0.0, 1.0)), SOSRI(), saveat = _saveat)
add_tstop!(integ, 2.0)
solve!(integ)
@test integ.sol.t == _saveat

integ = init(SDEProblem((u, p, t) -> u, (u, p, t) -> u, 0.0, (0.0, 1.0)),
    SOSRI(), saveat = _saveat, save_end = true)
add_tstop!(integ, 2.0)
solve!(integ)
@test integ.sol.t == [0.0, 0.25, 0.5, 1.0, 2.0]

integ = init(SDEProblem((u, p, t) -> u, (u, p, t) -> u, 0.0, (0.0, 1.0)),
    SOSRI(), saveat = _saveat, save_end = false)
add_tstop!(integ, 2.0)
solve!(integ)
@test integ.sol.t == _saveat
