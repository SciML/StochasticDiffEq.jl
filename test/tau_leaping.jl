...
sol2 = solve(EnsembleProblem(jump_iipprob), TauLeaping(); dt = 1.0, adaptive = false,
             save_everystep = false, trajectories = N)
...
sol2 = solve(EnsembleProblem(jump_prob), TauLeaping(); dt = 1.0, adaptive = false,
             save_everystep = false, trajectories = N)
mean2 = mean([sol2[i][end, end] for i in 1:N])
@test mean1≈mean2 rtol=1e-2
