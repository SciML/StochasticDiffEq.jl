...
# ThetaTrapezoidalTauLeaping: Weak second order tau-leaping method (Hu, Li, Min 2011)
...
# Test basic solve
@time sol_theta = solve(jump_iipprob_theta, ThetaTrapezoidalTauLeaping(); dt = 1.0,
    adaptive = false)

# Test with different theta values
for theta in [0.25, 0.5, 0.75]
    @time sol = solve(jump_iipprob_theta, ThetaTrapezoidalTauLeaping(theta = theta);
...
N_theta = 10_000
sol_tauleaping_theta = solve(EnsembleProblem(jump_iipprob_theta), TauLeaping(); dt = 1.0,
    adaptive = false, save_everystep = false, trajectories = N_theta)
sol_theta_ens = solve(EnsembleProblem(jump_iipprob_theta), ThetaTrapezoidalTauLeaping();
...
@test mean_tauleaping_theta ≈ mean_theta_ens rtol = 5e-2
...
@time sol_oop = solve(jump_prob_oop_theta, ThetaTrapezoidalTauLeaping(); dt = 1.0,
    adaptive = false)
@test length(sol_oop.t) > 0

# Ensemble test for out-of-place
sol_theta_oop_ens = solve(EnsembleProblem(jump_prob_oop_theta),
...
@test mean_theta_oop ≈ mean_theta_ens rtol = 5e-2
