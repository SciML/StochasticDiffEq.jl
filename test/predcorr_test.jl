...
prob = SDEProblem{false}(f,g,u0,(0.0,1.0),noise_rate_prototype=zeros(2,2))
probiip = SDEProblem{true}(f,g,u0,(0.0,1.0),noise_rate_prototype=zeros(2,2))

## use BenchmarkTools to benchmark speed here if desired.
using BenchmarkTools
dt = 0.002
seed = 1
solEMiip = @btime solve(probiip, EM(), dt=dt, seed=seed)
solRKMil = @btime solve(probiip, RKMilCommute(), dt=dt, seed=seed)
solPCEuler = @btime solve(prob, PCEuler(bbprime), dt=dt, seed=seed)
solPCEuleriip = @btime solve(probiip, PCEuler(bbprime), dt=dt, seed=seed)
