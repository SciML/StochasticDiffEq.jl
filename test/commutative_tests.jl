...
sim2 = test_convergence(dts, prob, RKMilGeneral(ii_approx = IICommutative()),
...
proboop = SDEProblem(ff_commute_oop, σ_oop, u0, (0.0, 1.0),
                     noise_rate_prototype = rand(2, 4))
...
sim2 = test_convergence(dts, proboop, RKMilGeneral(ii_approx = IICommutative()),
                        trajectories = Int(2e2))
@test abs(sim2.𝒪est[:final] - 1.0) < 0.2
